...
int open_clientfd(char *hostname, char *port) {
    ...
    hints.ai_socktype = SOCK_STREAM;  /* Open a connection *//*设置套接字类型为 流式套接字*/
    hints.ai_flags = AI_NUMERICSERV;  /* ... using a numeric port arg. *//*设置标志位，表示端口号是数值形式*/
    hints.ai_flags |= AI_ADDRCONFIG;  /* Recommended for connections *//*按位或  添加AI_ADDRCONFIG标志位，表示只返回与本地系统配置兼容的地址*/

    if ((rc = getaddrinfo(hostname, port, &hints, &listp)) != 0) {/*错误处理 */
    ...
    for (p = listp; p; p = p->ai_next) {
        ...
        if ((clientfd = socket(p->ai_family, p->ai_socktype, p->ai_protocol)) < 0) 
            continue; /* Socket failed, try the next 创建套接字失败，尝试下一个*/

        /* Connect to the server */
        if (connect(clientfd, p->ai_addr, p->ai_addrlen) != -1) 
            break; /* Success 连接成功*/
        if (close(clientfd) < 0) { /* Connect failed, try another 连接失败，尝试下一个*/  //line:netp:openclientfd:closefd      
